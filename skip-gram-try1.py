...
def train():
  ...
  with graph.as_default():
      ...
      word2Vec_model = model(batch_size, reverse_dictionary, reverse_dictionary1, vocabulary_size, vocabulary_size1, sd1_size, embedding_size, num_sampled)
              ...
              if step % 50000 == 0:
                  ...
                  for i in xrange(valid_size):
                      ...
                      for k in xrange(top_k):
                          ...
                          print(log_str)
                  saver.save(session, 'checkpoints-new/model.ckpt', global_step = step)
                  print('saved!')
...
def predict():
    ...
    weight_file = './checkpoints-new/model.ckpt-550000'
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    if weight_file is not None:
        saver.restore(sess, weight_file)
        print('Restoring weight file from:%s'%weight_file)
    f = open('result-new.txt','w+')
    start = time.time()
    vec = []
    batch_inputs = np.ndarray(shape=(batch_size),dtype=np.int32)
