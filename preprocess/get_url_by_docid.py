...
#from doc_base db left join doc_extrainfo dx on db.docid=dx.docid where db.docid='%s'" %(docid)
	...
	#print(selectResultList3[0][0])
        ...
        if(len(selectResultList1) > 0 and len(selectResultList1[0]) > 0):
            result_category[docid] = selectResultList1[0][0]
            print(selectResultList1[0][0])

        if(len(selectResultList2) > 0 and len(selectResultList2[0][0]) > 0):
            result_interests[docid] = selectResultList2[0][0]
            print(selectResultList2[0][0])
...
def main():
        ...
        with open('category1.json','w') as f:
            f.write(json.dumps(result_category,ensure_ascii=False))
        with open('interests.json','w') as ff:
            ff.write(json.dumps(result_interests,ensure_ascii=False))
        ...
        with open('category.json','w') as f:
            f.write(json.dumps(result_category))
        with open('interests.json','w') as ff:
            ff.write(json.dumps(result_interests))
