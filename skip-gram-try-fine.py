...
def predict():
    ...
    weight_file = './checkpoints-hour/model.ckpt-550000'
    sess = tf.Session()
...
def get_top_k(user_vec, vid_list):
    ...
    try:
        D,I = gpu_index_flat.search(user_vec,51)
        for m in range(1):
            tmp_res = []
            for n in range(1,51):
...
def calculate_all():
    for k,v in test_result.items():
        all_ap = all_ap + AP(set(v['predict']), set(v['hit']))
        all_ndcg = all_ndcg + NDCG(set(v['predict']),set(v['hit']))
...
def test():
    global test_result
    with open('result-hour.txt') as f:
        vid = f.readlines()
    vec = np.load('test-hour.npy',allow_pickle=True)
    vid_vec = {}
    print(vec.shape)
    vids = []
    for id in vid:
    ...
    for k,v in content.items():
        ...
        if num == 0:
            ...
            i = i + 1
            vec_ave = vec_all/num
            temp_res = get_top_k(vec_ave, vids)
            tmp = {}
